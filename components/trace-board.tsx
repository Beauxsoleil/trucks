...
export function TraceBoard({letter,onComplete}:{letter:Letter;onComplete:()=>void}){
  ...
  const progress=useRef(newGuidedTrace()),pointer=useRef<number|null>(null);
  ...
  function release(event:PointerEvent<SVGSVGElement>){
    ...
    pointer.current=null;progress.current={...progress.current,previous:null};
  ...
  return <div className={`trace-board ${view.complete?'is-complete':''}`} data-complete={view.complete} data-active-stroke={view.stroke}>
    ...
    }} onPointerMove={event=>{if(pointer.current===event.pointerId)move(event)}} onPointerUp={release} onPointerCancel={release} onLostPointerCapture={()=>{pointer.current=null;progress.current={...progress.current,previous:null}}}>
      <title>Trace {letter.name} in order</title><desc>Start at the numbered dot and follow the arrow. Lift your finger whenever you need to, then continue near the blue dot. Move to the next number for the next step.</desc>
